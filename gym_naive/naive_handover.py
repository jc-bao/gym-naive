...
class NaiveHandover(gym.Env):
    def __init__(self, reward_type='dense', dim = 3):
        self.dim = dim
        self.reward_type = reward_type
        self._max_episode_steps = 50
        self.obj_length = 0.5 # object length
        self.error = 0.05
        self.space = spaces.Box(low=-np.ones(dim), high=np.ones(dim))
        '''
        Observation Space
        [0:dim] agent1 pos
        [dim+1,dim*2] agent2 pos
        [dim*2+1, dim*3] object center pos
        [dim*3+1, dim*4] goal pos
        '''
        self.observation_space = spaces.Box(low=-np.ones(dim*4), high=np.ones(dim*4))
        '''
        Action Space
        [0:dim] agent1 position
        [dim] agent1 gripper
        [dim+1:dim*2+1] agent2 position
        [dim*2+1] agent2 gripper
        '''
        self.action_space = spaces.Box(low=-np.ones((dim+1)*2), high=np.ones((dim+1)*2))
        self.reset()
    ...
    def reset(self):
        ...
        self.agent1 = self.space.sample()
        self.agent2 = self.space.sample()
...
register(
    id='NaiveHandover-v0',
    entry_point='gym_naive.naive_handover:NaiveHandover',
)
